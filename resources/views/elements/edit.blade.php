@php
    ...
    $selectedElementType = old('type', $element->type->value);
@endphp
    ...
    <div class="py-10">
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white border border-opta-teal-light/30 shadow-sm sm:rounded-xl p-6 text-left">
                <form method="post" action="{{ route('groups.elements.update', [$group, $element]) }}" class="space-y-4" x-data="{ type: @js($selectedElementType) }">
                    @csrf
                    @method('PATCH')
                    @if ($errors->any())
                    ...
                    <fieldset class="rounded-lg border border-opta-teal-light/30 p-4 space-y-3 transition-opacity" :class="type !== '{{ ElementType::Screenshot->value }}' ? 'opacity-50' : ''">
                        ...
                        <div>
                            ...
                            <x-text-input id="screenshot_url" name="config[url]" type="url" class="mt-1 block w-full" :value="old('config.url', $element->config['url'] ?? '')" placeholder="https://example.com" />
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            ...
                            <div>
                                <x-input-label for="screenshot_password" class="inline-flex items-center gap-1">
                                    <span>{{ __('Kennwort (optional, leer lassen = unverändert)') }}</span>
                        ...
                        <div>
                            <x-input-label for="screenshot_totp" class="inline-flex items-center gap-1">
                                <span>{{ __('TOTP-Secret (Base32, optional, leer lassen = unverändert)') }}</span>
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                ...
                                <x-text-input id="sel_username" name="config[selectors][username]" type="text" class="mt-1 block w-full font-mono text-xs" :value="old('config.selectors.username', $element->config['selectors']['username'] ?? '')" placeholder="#email" />
                            </div>
                            <div>
                                ...
                                <x-text-input id="sel_password" name="config[selectors][password]" type="text" class="mt-1 block w-full font-mono text-xs" :value="old('config.selectors.password', $element->config['selectors']['password'] ?? '')" placeholder="#password" />
                            </div>
                            <div>
                                ...
                                <x-text-input id="sel_totp" name="config[selectors][totp]" type="text" class="mt-1 block w-full font-mono text-xs" :value="old('config.selectors.totp', $element->config['selectors']['totp'] ?? '')" placeholder="#totp" />
                            </div>
                            <div>
                                ...
                                <x-text-input id="sel_submit" name="config[selectors][submit]" type="text" class="mt-1 block w-full font-mono text-xs" :value="old('config.selectors.submit', $element->config['selectors']['submit'] ?? '')" placeholder="button[type=submit]" />
                            </div>
                            <div>
                                ...
                                <x-text-input id="sel_totp_submit" name="config[selectors][totp_submit]" type="text" class="mt-1 block w-full font-mono text-xs" :value="old('config.selectors.totp_submit', $element->config['selectors']['totp_submit'] ?? '')" placeholder="#verify-otp" />
                            </div>
                        </div>
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                            <div>
                                ...
                                <x-text-input id="wait_for" name="config[wait_for]" type="text" class="mt-1 block w-full font-mono text-xs" :value="old('config.wait_for', $element->config['wait_for'] ?? '')" placeholder="main" />
                            </div>
                            <div>
                                ...
                                <x-text-input id="timeout_ms" name="config[timeout_ms]" type="number" class="mt-1 block w-full" :value="old('config.timeout_ms', $element->config['timeout_ms'] ?? '')" placeholder="60000" min="1" max="300000" />
                            </div>
                        </div>
                        <div class="flex items-center gap-2 pt-1">
                            <input type="hidden" name="config[full_page]" value="0" />
                            <input id="screenshot_full_page" name="config[full_page]" type="checkbox" value="1" class="rounded border-opta-teal-light text-opta-teal-dark focus:ring-opta-teal-dark" @checked(old('config.full_page', (bool) ($element->config['full_page'] ?? true))) />
                    ...
                    <div>
                        ...
                        <textarea id="config" name="config_json" rows="4" class="mt-1 block w-full rounded-md border-opta-teal-light/50 shadow-sm focus:border-opta-teal-dark focus:ring-opta-teal-dark font-mono text-xs">{{ old('config_json', json_encode($element->config ?? [], JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE)) }}</textarea>
                    ...
                    @if(auth()->user()->can('setConsumerApiFlag', [$element, $group]))
                        <div class="flex items-center gap-2">
                            ...
                            <x-input-label for="consumer_can_read_via_api" class="!mb-0 inline-flex items-center gap-1">
                                <span>{{ __('Consumer darf per API lesen (diese Gruppe)') }}</span>
