                    ...
                    <div class="space-y-6">
                        ...
                        <div class="grid gap-6 sm:grid-cols-2">
                            @foreach ($group->elements as $el)
                                @php
                                    $elementOpenUrl = null;
                                    if ($el->type === ElementType::Screenshot || $el->type === ElementType::Scrape) {
                                        $rawUrl = $el->config['url'] ?? null;
                                        if (is_string($rawUrl) && trim($rawUrl) !== '') {
                                            $elementOpenUrl = trim($rawUrl);
                                        }
                                    }
                                @endphp
                                <article
                                ...
                                >
                                    <div class="flex flex-wrap items-start justify-between gap-3">
                                        ...
                                        <div class="flex flex-wrap items-center gap-1 sm:gap-2 shrink-0">
                                            @if ($elementOpenUrl !== null)
                                                <a
                                                    href="{{ $elementOpenUrl }}"
                                                    target="_blank"
                                                    rel="noopener noreferrer"
                                                    class="inline-flex items-center justify-center rounded-lg p-2 text-opta-teal-dark hover:bg-opta-teal-light/20 focus:outline-none focus:ring-2 focus:ring-opta-teal-dark"
                                                    title="{{ __('URL in neuem Tab öffnen') }}"
                                                    aria-label="{{ __('URL in neuem Tab öffnen') }}"
                                                >
                                            ...
                                            @can('update', $el)
                                                <a
                                                    href="{{ route('groups.elements.edit', [$group, $el]) }}"
                                                    class="inline-flex items-center justify-center rounded-lg p-2 text-opta-teal-dark hover:bg-opta-teal-light/20 focus:outline-none focus:ring-2 focus:ring-opta-teal-dark"
                                                    title="{{ __('Bearbeiten') }}"
                                                ...
                                                <form
                                                    method="post"
                                                    action="{{ route('groups.elements.destroy', [$group, $el]) }}"
                                                    class="inline-flex"
                                                    onsubmit="return confirm(@json(__('Element von dieser Gruppe entfernen?')));"
                                                >
                                                    @csrf
                                                    @method('DELETE')
                                                    <button
                                                        type="submit"
                                                        class="inline-flex items-center justify-center rounded-lg p-2 text-opta-berry hover:bg-opta-berry/10 focus:outline-none focus:ring-2 focus:ring-opta-berry"
                                                        title="{{ __('Entfernen') }}"
